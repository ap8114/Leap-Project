      ...
      <div className="container py-5">
        ...
        <div className="card shadow-sm mb-5">
          <div className="card-body p-5">
            ...
            >
              <div className="mb-4">
                <FontAwesomeIcon icon={faCloudUploadAlt} className="text-muted mb-3" size="3x" />
              </div>
              <h3 className="h4 fw-semibold text-dark mb-2">Drag & Drop files here</h3>
              ...
              <div className="mt-3">
                <p className="small text-muted">Supported formats: PDF, Word (.doc, .docx)</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="d-flex bg-light rounded-3 p-1">
            {categories.map((category) => (
              <button
                ...
                className={`btn ${activeCategory === category ? 'btn-white shadow-sm text-primary' : 'text-muted'} flex-grow-0 mx-1`}
              >
                {category}
              </button>
            ))}
        ...
        {filteredDocuments.length > 0 ? (
                  ...
                  <div className="card-body">
                    <div className="d-flex justify-content-between mb-3">
                      <div className="d-flex align-items-start">
                        ...
                        />
                        <div>
                          <h5 className="fw-semibold text-dark mb-1 small">{document.name}</h5>
                          <div className="d-flex align-items-center mt-2">
                            <span className={`badge ${document.type === 'PDF' ? 'bg-danger-light text-danger' : 'bg-primary-light text-primary'} me-2`}>
                              {document.type}
                            </span>
                            <span className="small text-muted">{document.size}</span>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          <div className="text-center py-5">
            <div className="mb-4">
              <FontAwesomeIcon icon={faFolderOpen} className="text-muted" size="4x" />
            </div>
            <h3 className="h4 fw-semibold text-dark mb-2">No {activeCategory.toLowerCase()} documents yet</h3>
            <p className="text-muted mb-4">Upload your first {activeCategory.toLowerCase()} document to get started</p>
            <label htmlFor="empty-state-upload" className="d-inline-block">
              <input
                ...
                className="d-none"
              />
              <span className="btn btn-primary px-4 py-2">
                Upload Document
              </span>
            </label>
          </div>
      ...
      <div className="modal-content">
        ...
        <div className="modal-header border-bottom">
          <div className="d-flex align-items-center flex-wrap">
            <FontAwesomeIcon
              ...
              className={`${getFileIconColor(selectedDocument.type)} me-3 fs-4`} // Smaller and responsive
            />
            <div>
              <h5 className="modal-title mb-1">{selectedDocument.name}</h5>
              <div className="d-flex align-items-center mt-1 gap-2 flex-wrap">
                <span className={`badge ${selectedDocument.type === 'PDF' ? 'bg-danger-light text-danger' : 'bg-primary-light text-primary'}`}>
                  {selectedDocument.type}
                </span>
                <span className="text-muted small">{selectedDocument.size}</span>
              </div>
            </div>
          </div>
        ...
        <div className="modal-body">
          <div className="row mb-4">
            <div className="col-sm-6 mb-3 mb-sm-0">
              ...
              <p className="fw-medium text-dark">{selectedDocument.category}</p>
            </div>
          ...
          <div className="bg-light rounded-3 p-3 mb-4">
            ...
            <div className="ratio ratio-16x9 bg-white rounded-3 d-flex align-items-center justify-content-center">
              <FontAwesomeIcon icon={faFileAlt} className="text-muted fs-2" /> {/* Smaller icon */}
            </div>
          </div>
        </div>
        ...
        <div className="modal-footer border-top bg-light">
          <div className="d-flex justify-content-end gap-2 flex-wrap">
            <button onClick={() => setIsModalOpen(false)} className="btn btn-outline-secondary">
              Close
            </button>
