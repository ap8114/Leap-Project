...
const ActivitiesTable = () => {
  ...
  useEffect(() => {
    const handleClick = (e) => {
      if (
        columnsRef.current &&
        !columnsRef.current.contains(e.target)
      ) {
          ...
          {visibleColumns.includes("Actions") && (
            <>
              ...
              <td>
               ...
               <td>
  <div className="btn-group">
    <button className="btn btn-light btn-sm fw-semibold">Edit</button>
    <button 
    ...
    <ul className="dropdown-menu">
      ...
      <li><button className="dropdown-item text-danger">Delete</button></li>
          ...
          {visibleColumns.includes("Type") && (
            <td>
              <BsPlay className="me-1" /> <span style={{ color: "#1976d2" }}>{row.time}</span>
            </td>
          )}
          ...
          {visibleColumns.includes("Non-billable ($)") && <td>{row.nonBillable}</td>}
          {visibleColumns.includes("Billable ($)") && <td>{row.billable}</td>}
          {visibleColumns.includes("Date") && <td>{row.date}</td>}
          {visibleColumns.includes("User") && (
            <td>
              <a href="#" style={{ color: "#1976d2", textDecoration: "underline" }}>{row.user}</a>
            </td>
          )}
          {visibleColumns.includes("Invoice status") && (
            <td>
              <span className="badge bg-light text-dark border px-3 py-1" style={{ fontWeight: 500, fontSize: 14 }}>
                {row.invoiceStatus}
              </span>
            </td>
          )}
    ...
    return (
      <tr>
        <td colSpan={visibleColumns.length + 1} style={{ height: 120, background: "#fff", position: "relative", padding: 0 }}>
          <div className="d-flex flex-column align-items-center justify-content-center" style={{ minHeight: 180 }}>
            <div style={{ position: "relative", marginBottom: 12 }}>
              <svg width="120" height="80" viewBox="0 0 90 60" fill="none">
                ...
                <path d="M50 18a12 12 0 1 1 0 24 12 12 0 0 1 0-24zm0 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20zm.75 4.5v6.25l5.25 5.25-1.5 1.5-6-6V24.5h2.25z" fill="#1976d2" />
              </svg>
              <span style={{
                position: "absolute",
                right: 10,
            ...
            <div>
              <h5 className="fw-bold mb-1" style={{ color: "#222", background: "#fff", display: "inline-block", padding: "0 6px" }}>
      ...
      >
        <span
          ...
          style={{
            color: color,
            fontSize: 18,
            ...(activeTab === tab && tab === "Expense" ? { color: "#7e3ff2" } : {}),
  ...
  return (
    <div style={{ background: "#f7fafd", minHeight: "100vh" }}>
      ...
      {/* Filters Row */}
      <div className="container-fluid px-2 px-md-4 py-3" style={{ background: "#f7fafd" }}>
        <div
        ...
        >
          {/* Tabs */}
            <div className="d-flex flex-nowrap gap-2" style={{ minWidth: 260 }}>
                {tabBtn("All", null, "All", "#1976d2")}
                {tabBtn("Time", <BsClock />, "Time", "#FFA646")}
                {tabBtn("Expense", <BsCurrencyDollar />, "Expense", "#7e3ff2")}
            </div>
          {/* Date Range */}
          <div className="input-group input-group-sm" style={{ width: 130, minWidth: 110 }}>
            <input type="text" className="form-control" value="07/10/2025" readOnly />
            <span className="input-group-text bg-white"><BsCalendar /></span>
          </div>
          ...
          {/* Pagination Arrows */}
          <button className="btn btn-light btn-sm border px-2"><BsChevronLeft /></button>
          <button className="btn btn-light btn-sm border px-2"><BsChevronRight /></button>
          {/* Today Dropdown */}
          <select className="form-select form-select-sm" style={{ width: 110, minWidth: 90 }}>
            <option>Today</option>
          ...
          <div className="dropdown" ref={columnsRef} style={{ position: "relative" }}>
            ...
            >
              <li className="mb-2 fw-bold" style={{ fontSize: 15 }}>Visible columns</li>
              {columnsList.map((col) => (
                <li key={col} className="mb-1">
                  <div className="form-check">
                    <input
                      ...
                      style={{
                        borderColor: "#1976d2",
                        backgroundColor: visibleColumns.includes(col) ? "#1976d2" : "#fff"
                      }}
                    />
                    <label className="form-check-label" htmlFor={`col-${col}`} style={{ fontSize: 15 }}>
                      {col}
                    </label>
                  </div>
                </li>
          ...
          {/* Filters Dropdown */}
          <div className="dropdown" ref={filtersRef} style={{ position: "relative" }}>
            <button
              className="btn btn-sm fw-bold d-flex align-items-center"
              type="button"
            ...
            >
              <BsFilter className="me-1" /> Filters <BsChevronDown className="ms-1" />
            </button>
            <div
              className="dropdown-menu p-3 border-0 shadow-sm"
              style={{
            ...
            >
              <div className="mb-2 fw-bold" style={{ fontSize: 15 }}>Firm user</div>
              <div className="input-group input-group-sm mb-3">
                <input type="text" className="form-control" value="aman patidar (me)" readOnly />
                <button className="btn btn-outline-secondary" tabIndex={-1} type="button">×</button>
              </div>
              <div className="mb-2 fw-bold" style={{ fontSize: 15 }}>Invoice status</div>
              <select className="form-select form-select-sm mb-3">
                <option>Select an option</option>
              </select>
              <div className="mb-2 fw-bold" style={{ fontSize: 15 }}>Matter</div>
              <div className="input-group input-group-sm mb-3">
                <input type="text" className="form-control" placeholder="Find a matter" />
                <button className="btn btn-outline-secondary dropdown-toggle" type="button" tabIndex={-1}></button>
              </div>
              <div className="mb-2 fw-bold" style={{ fontSize: 15 }}>Expense attachments</div>
              <select className="form-select form-select-sm mb-3">
                <option>Select an option</option>
              </select>
              <div className="mb-2 fw-bold" style={{ fontSize: 15 }}>Time entry category</div>
              <div className="input-group input-group-sm mb-3">
                <input type="text" className="form-control" placeholder="Find a time entry category" />
                <button className="btn btn-outline-secondary dropdown-toggle" type="button" tabIndex={-1}></button>
              </div>
              <div className="mb-2 fw-bold" style={{ fontSize: 15 }}>Expense category</div>
              <div className="input-group input-group-sm mb-3">
                <input type="text" className="form-control" placeholder="Find an expense category" />
                <button className="btn btn-outline-secondary dropdown-toggle" type="button" tabIndex={-1}></button>
              </div>
      ...
      {/* Table */}
      <div className="container-fluid px-0 px-md-4 pb-3" style={responsiveContainer}>
        <div className="at-table-container table-responsive bg-white rounded-3 border" style={{ minHeight: 400 }}>
          <table className="at-table table table-bordered mb-0" style={{ minWidth: 900 }}>
            <thead>
              <tr className="table-light align-middle">
                {visibleColumns.includes("Actions") && <th style={{ width: 30 }}><input type="checkbox" /></th>}
                ...
                {visibleColumns.includes("Non-billable ($)") && <th>Non-billable ($)</th>}
                {visibleColumns.includes("Billable ($)") && <th>Billable ($)</th>}
                {visibleColumns.includes("Date") && <th>Date</th>}
                {visibleColumns.includes("User") && <th>User</th>}
                {visibleColumns.includes("Invoice status") && <th>Invoice status</th>}
                {visibleColumns.includes("Expense Category") && <th>Expense Category</th>}
...
<div className="container-fluid px-2 px-md-4 pb-4">
  <div className="at-footer-actions d-flex flex-column flex-md-row justify-content-between align-items-center mt-2 gap-2">
    <div className="d-flex align-items-center gap-2 mb-2 mb-md-0">
      <button className="btn btn-light btn-sm border"><BsChevronLeft /></button>
      <button className="btn btn-light btn-sm border"><BsChevronRight /></button>
